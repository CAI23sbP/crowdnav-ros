...
class RGL(nn.Module):
    ...
    def compute_similarity_matrix(self, X):
        if self.similarity_function == 'embedded_gaussian': ## paper
    ...
    def forward(self, state):
        ...
        X = torch.cat([robot_state_embedings, human_state_embedings], dim=1) # 1,2,3

        # compute matrix A
        if not self.layerwise_graph:
            normalized_A = self.compute_similarity_matrix(X) # 1,2,3
            self.A = normalized_A[0, :, :].data.cpu().numpy() # 2,3
